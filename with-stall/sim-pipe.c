...
void do_stall() {
	// FIXME: debug
	fprintf(stdout, "[DEBUG]In do stall de.in1: %d, de.in2: %d, em.regDst: %d\n", 
			de.oprand.in1, de.oprand.in2, em.regDst);
	if ((em.signal.regwrite && (REG_HAZARD(de.oprand.in1, em.regDst) || REG_HAZARD(de.oprand.in2, em.regDst))) ||
...
void do_forward() {
	/* detect data hazard and do forwarding and stall */
	int set_oprand_1 = 0;
	int set_oprand_2 = 0;
	forwarding_unit(&set_oprand_1, &set_oprand_2, em.signal.regwrite, mw.signal.regwrite, 
			de.oprand.in1, de.oprand.in2, em.regDst, mw.regDst);
	// FIXME: debug
	PRINT_FORWARDING_SIG(stdout, set_oprand_1, set_oprand_2);
	/* first do data forwarding */
	if (set_oprand_1) {
		if (set_oprand_1 == 1 && !em.signal.memtoreg) {
			set_oprand_1 = 0;		/* set hazard signal to zero */
			de.reg_data_1 = em.ALUresult;		/* set oprand_1 with forwarding value */
		}
		if (set_oprand_1 == 2) {
			set_oprand_1 = 0;		/* set hazard signal to zero */
			de.reg_data_1 = mw.signal.memtoreg ? mw.mem_data : mw.alu_data;		/* set oprand_1 with forwarding value */
		}
	}
	if (set_oprand_2) {
		if (set_oprand_2 == 1 && !em.signal.memtoreg) {
			set_oprand_2 = 0;		/* set hazard signal to zero */
			de.reg_data_2 = em.ALUresult;		/* set oprand_1 with forwarding value */
		}
		if (set_oprand_2 == 2) {
			set_oprand_2 = 0;		/* set hazard signal to zero */
			de.reg_data_2 = mw.signal.memtoreg ? mw.mem_data : mw.alu_data;		/* set oprand_1 with forwarding value */
		}
	}
	/* if forwarding can`t solve problem, then do stall */
	if (set_oprand_1 || set_oprand_2) {
		PRINT_INFO(stdout, "IN EX, do stall\n")
		do_stall();
	}
}
...
READ_OPRAND_VALUE: 
	{
		...
		switch(de.opcode){
			...
			case LUI:
				de.reg_data_1 = 0;
				de.reg_data_2 = 0;
				de.imm = UIMM; // FIXME: LUI semantic is different from LUI_IMPL in machine.def
				de.reg_dst_rt = de.oprand.out1;
				de.reg_dst_rd = 0;
				break;
			case SW:
